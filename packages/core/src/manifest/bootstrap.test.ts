  ...
  it("catches a prefix or envelope-kind branch that derives caller-selected bootstrap paths", () => {
    expect(deriveBootstrapEnvelopePaths(productHome, "fresh_v2_init", freshId)).toEqual({
      ...
      journalSlots: [
        ...
        "/product/state/fresh-v2-init.fi_123e4567-e89b-42d3-a456-426614174000.journal.1.json",
      ],
...
function fullPlanFixture() {
  ...
  const plan: FreshV2InitPlanV1 = {
    ...
    journalSlots: [
      { slot: 0, path: `/product/state/fresh-v2-init.${freshId}.journal.0.json` as never, ownerUid: 501, mode: 0o600, nlink: 1, dev: "1" as never, ino: "11" as never },
      { slot: 1, path: `/product/state/fresh-v2-init.${freshId}.journal.1.json` as never, ownerUid: 501, mode: 0o600, nlink: 1, dev: "1" as never, ino: "12" as never },
    ],
...
describe("retained bootstrap persisted envelope", () => {
  function retainedPlanFixture(): {
    ...
    candidate.journalSlots = [
      { slot: 0, path: `/product/state/fresh-v2-init.${freshId}.journal.0.json`, ownerUid: 501, mode: 0o600, nlink: 1, dev: "1", ino: "40" },
  ...
  it("admits exactly two identity-bound journal slots instead of a singular journal path", () => {
    const fixture = retainedPlanFixture();
  ...
  it("admits only the two-slot retained journal record shape", () => {
    ...
    expect(validateBootstrapJournal(plan, planned)).toStrictEqual(planned);
    expect(() => validateBootstrapJournal(plan, {
      ...planned,
      phase: "compacting",
      compactionNext: 0,
  ...
  it("admits the exact retention-only terminal preimage shape and refuses every open form", () => {
    const { plan } = retainedPlanFixture();
    const terminalPreimage = {
      previousJournalHash: createHash("sha256").update("terminal-predecessor").digest("hex"),
    ...
    const retaining = {
      ...
      slot: 1,
      sequence: "3",
      previousJournalHash: createHash("sha256").update("terminal-record").digest("hex"),
      phase: "retaining",
      direction: "forward",
      nextPayload: plan.payloads.length,
      payloadWriteState: { state: "idle" },
      nextCreatedPath: plan.createdPaths.length,
      nextFoundationParticipant: plan.foundationParticipants.filter((participant) =>
        participant.role.kind === "forward",
      ).length,
      ...
      retentionNext: 0,
      retentionTerminalPreimage: terminalPreimage,
      createdAt: "2026-08-31T12:00:00.000Z",
      updatedAt: "2026-08-31T12:00:02.000Z",
    } as const;

    expect(validateBootstrapJournal(plan, retaining)).toStrictEqual(retaining);
    const missing = structuredClone(retaining) as Partial<typeof retaining>;
    Reflect.deleteProperty(missing, "retentionTerminalPreimage");
    expect(() => validateBootstrapJournal(plan, missing)).toThrow(BootstrapStateError);
    expect(() => validateBootstrapJournal(plan, {
      ...retaining,
      retentionTerminalPreimage: { ...terminalPreimage, extra: "open" },
    })).toThrow(BootstrapStateError);
    expect(() => validateBootstrapJournal(plan, {
      ...retaining,
      retentionTerminalPreimage: { ...terminalPreimage, updatedAt: "not-a-timestamp" },
    })).toThrow(BootstrapStateError);
    expect(() => validateBootstrapJournal(plan, {
      ...retaining,
      retentionTerminalPreimage: { ...terminalPreimage, previousJournalHash: "not-a-hash" },
    })).toThrow(BootstrapStateError);
    expect(() => validateBootstrapJournal(plan, {
      ...retaining,
      phase: "finalized",
...
function admittedJournalPlan(): FreshV2InitPlanV1 {
  return {
    ...
    journalSlots: [
      { slot: 0, path: "/product/state/fresh-v2-init.fi_123e4567-e89b-42d3-a456-426614174000.journal.0.json" as never, ownerUid: 501, mode: 0o600, nlink: 1, dev: "1" as never, ino: "11" as never },
      { slot: 1, path: "/product/state/fresh-v2-init.fi_123e4567-e89b-42d3-a456-426614174000.journal.1.json" as never, ownerUid: 501, mode: 0o600, nlink: 1, dev: "1" as never, ino: "12" as never },
    ],
...
const admittedPlanHash = createHash("sha256")
  .update(encodeCanonicalJson(admittedJournalPlan() as never))
  .digest("hex") as LowerHexSha256;

function journal(overrides: Partial<FreshV2InitJournalV1> = {}): FreshV2InitJournalV1 {
  const initial = (overrides.phase ?? "planned") === "planned";
  return {
    schemaVersion: 1,
    id: freshId,
    planHash: admittedPlanHash,
    slot: 0,
    sequence: (initial ? "0" : "2") as never,
    previousJournalHash: initial ? null : hashA,
    phase: "planned",
    direction: "forward",
    nextPayload: 0,
    payloadWriteState: { state: "idle" },
...
const legalForwardJournalRows: readonly [string, Partial<FreshV2InitJournalV1>][] = [
  ...
  [
    "finalized retention starts from the complete force-forward cursor",
    {
      phase: "retaining",
      nextPayload: 1,
      nextCreatedPath: 1,
      ...
      retentionTerminalPreimage: {
        previousJournalHash: hashA,
        updatedAt: "2026-08-29T12:00:00.000Z" as never,
      },
    },
  ],
  [
    "rolled-back retention retains the compensating direction and closed reverse cursor",
    {
      phase: "retaining",
      direction: "compensating",
      nextPayload: 1,
...
const illegalCursorRows: readonly [string, Partial<FreshV2InitJournalV1>][] = [
  ...
  ["a retention cursor appears outside retaining", { retentionNext: 0 }],
  [
  ...
  [
    "retention advances beyond its global bound",
    {
      phase: "retaining",
      nextPayload: 1,
      nextCreatedPath: 1,
