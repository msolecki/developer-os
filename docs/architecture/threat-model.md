...
| A V1 manifest is refused, never migrated, once the packaged bootstrap capability exists (Spec 2's D18 amendment) | `init` returns reason `manifest_v1_not_migratable`, exit 4, recovery `developer-os uninstall, then developer-os init`, before the bootstrap evidence inventory or any plan (`apps/cli/src/commands/init.ts:817-825`); the refusal is `apps/cli/src/bootstrap/report.ts:168-177`. Without the capability — production until roadmap Phase 4b — the V1 `init` path is unchanged | `apps/cli/src/commands/init.test.ts` — `refuses a shipped V1 installation once the packaged capability is available, and keeps the V1 path without it`, whose V1 home comes from the shipped V1 `init` and whose tree digest is compared across a dry and a real run |
| No command but `init` runs over an interrupted bootstrap, and retained evidence after the handoff blocks nothing | dispatch reads the bounded closure before any other command (`apps/cli/src/main.ts:514-524`): each admitted plan, its plan-bound slots and the selected journal, never a tombstone (`apps/cli/src/bootstrap/report.ts:738-784`), and refuses exit 6 with reason `bootstrap_recovery_required` while any envelope has no terminal outcome or cannot be read (`apps/cli/src/bootstrap/report.ts:1293-1315`) | `apps/cli/src/main.test.ts` — `refuses every non-init command while a fresh V2 envelope is non-terminal, and none after init completes the handoff`, nine commands with the tree digest unchanged and none refused by the gate once `init` resumes; `tests/security/network.test.ts` — `spawn nothing when every non-init command refuses a non-terminal envelope` |
| Spec 1 authority is admitted only over the exact V2 handoff, never a partial or repaired one | `admitV2Handoff` (`apps/cli/src/bootstrap/report.ts:1361-1428`) pins the current manifest to exactly one `finalized` envelope through the existing `exactV2Handoff` (`apps/cli/src/bootstrap/report.ts:604-629`), then refuses V2 drift, a missing nonce, allocator or global lock, nonce/allocator disagreement, a present activation, rollback or recovery-executor record, and a missing or non-empty ledger or rollback root; a V1 manifest gets the exit-4 refusal above, everything else exit 6 | `apps/cli/src/bootstrap/report.test.ts` — `admits exactly the complete handoff and refuses a non-terminal envelope and each missing member`, one refusal per member between two successful admissions, and `refuses a manifest the shipped V1 init produced` |
