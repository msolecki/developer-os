...
| Boundary | Mechanism | Evidence |
|---|---|---|
| The model is invoked with zero declared write scopes | `writeScopes: []` at the call site (`apps/cli/src/commands/ingest.ts:796`); Codex derives `-s read-only` **from the count and never from an argument**, which is what makes `danger-full-access` unreachable rather than merely unwritten (`packages/adapter-codex/src/invoke.ts:229-232`, `packages/adapter-codex/src/invoke.ts:302`); Claude is passed `--tools ""` — an empty tool *set*, not an allow-list — so there is no tool the vendor's own permission system could grant a write through, and no `allowedTools`-shaped field exists on `ClaudeInvocation` for a caller to widen (`packages/adapter-claude/src/invoke.ts:7-11,122`) | `apps/cli/src/commands/ingest.test.ts:1692` — `gives claude no tools at all, so the read scope is the prompt and nothing else`; `packages/adapter-claude/src/invoke.test.ts:54` — `passes argv as an array, in print mode, with no tools and no user configuration`; `packages/adapter-codex/src/invoke.test.ts:314` — `uses read-only and adds no --add-dir when there are no write scopes`; `tests/security/network.test.ts:212` — `spawns exactly one process during ingest, and it is the discovered vendor binary` |
...
| A V1 manifest is refused, never migrated, once the packaged bootstrap capability exists (Spec 2's D18 amendment) | `init` publishes reason `manifest_v1_not_migratable`, exit 4, recovery `developer-os uninstall, then developer-os init`, through `failureFrom`, before the bootstrap evidence inventory or any plan (`apps/cli/src/commands/init.ts:824-827`); the refusal's `name` is spelled so that `failureFrom` derives exactly that reason on any path (`apps/cli/src/bootstrap/report.ts:187-197`). Without the capability — production until roadmap Phase 4b — the V1 `init` path is unchanged | `apps/cli/src/commands/init.test.ts` — `refuses a shipped V1 installation once the packaged capability is available, and keeps the V1 path without it`, whose V1 home comes from the shipped V1 `init` and whose tree digest is compared across a dry and a real run; `apps/cli/src/bootstrap/report.test.ts` — `refuses a manifest the shipped V1 init produced`, which publishes the admission's rejection through `failureFrom` |
| No command but `init` runs over an interrupted or malformed bootstrap or a malformed V2 manifest, and retained evidence after a valid V2 handoff blocks nothing | dispatch routes every other command first (`apps/cli/src/main.ts:514-525`). A manifest declaring `schemaVersion: 2` must pass Core's structural V2 validation of its bytes with an unconfined owner-path admission, reading neither `config.toml` nor the Brain path (`apps/cli/src/bootstrap/report.ts:1354-1369`); otherwise every command exits 6 as malformed manifest state, and `init` refuses with the same message from the same check (`apps/cli/src/commands/init.ts:828-833`). An unparseable configuration or a relocated Brain is left to `doctor` and to each command's own confined read. With a valid V2 manifest, the gate reads only `fresh-v2-init.<id>.plan.json` names and never stats a tombstone, payload or lock name (`apps/cli/src/bootstrap/report.ts:1324-1343`); it refuses only a readable non-terminal journal whose plan published the current manifest. Without a version-2 manifest, it refuses on the evidence inspection `init` runs — resume, or manual archive including an unreadable namespace (`apps/cli/src/bootstrap/report.ts:1371-1399`) — and `init` gives the same archive refusal when that inspection cannot read the namespace (`apps/cli/src/commands/init.ts:840-851`). Known limitation: a crash after manifest publication and before verification, followed by journal-slot corruption, is indistinguishable from a tampered finalized install and is admitted, as is a plan that no longer admits or a still-valid manifest no longer matching the interrupted plan | `apps/cli/src/main.test.ts` — `admits every ordinary command when retained evidence goes missing or is altered after a complete V2 handoff` (missing and symlinked tombstones, a symlink inside a retained directory, a symlinked bootstrap lock, emptied, oversized and replaced slots); `refuses every command, and init, when a planted manifest declaring schema version 2 fails strict validation`; `refuses every command, and init, when a shipped V2 manifest is altered into an invalid one`; `does not refuse a valid V2 install as a malformed manifest when its configuration is unparseable or its Brain moved`; `refuses every non-init command with init's own archive guidance when an interrupted envelope's plan or slot stops matching`; `refuses only genuine bootstrap residue in a home where bootstrap never ran, with the guidance init gives`; `refuses every non-init command while a fresh V2 envelope is non-terminal, and none after init completes the handoff`; `tests/security/network.test.ts` — `spawn nothing when every non-init command refuses a non-terminal envelope` |
| Spec 1 authority is admitted only over the exact V2 handoff, never a partial or repaired one | `admitV2Handoff` (`apps/cli/src/bootstrap/report.ts`) pins the current manifest to exactly one `finalized` envelope through the existing `exactV2Handoff` (`apps/cli/src/bootstrap/report.ts:624-649`). It refuses managed drift with exit 3 (`apps/cli/src/bootstrap/report.ts:178-185`), including a missing rollback root, because drift runs first. With exit 6 it refuses a finding on the nonce, allocator, active-release or trust record, a missing global lock, nonce/allocator disagreement, a present activation record, a non-empty rollback or recovery-executor reservation, a non-empty ledger or rollback root, and — since Spec 1 §2.1's A12 amendment took the bookkeeping set out of the manifest, so §2.4 closure rather than drift owns the ledger roots — a missing ledger root. A V1 manifest gets the exit-4 refusal above. The zero-byte rollback and executor reservations fresh `init` creates are admitted as absent records | `apps/cli/src/bootstrap/report.test.ts` — `admits exactly the complete handoff and refuses a non-terminal envelope and each missing member`, one refusal per member with its exit code between two successful admissions, and `refuses a manifest the shipped V1 init produced` |
