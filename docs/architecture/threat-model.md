...
other seven use (§5.7, §5.12). None was fixed
...
| **Credentials** | no Keychain, no token store; the protected-path policy refuses `.ssh`, `.aws`, `.gnupg`, `.env` and `.env.*`, and three exact files, on both the declared and the canonical path (`packages/security/src/protected-paths.ts:9-19,129-137`) | `packages/security/src/protected-paths.test.ts:130` — `rejects an innocent alias that resolves into synthetic SSH data`; `:109` — `rejects an innocent alias outside home that resolves to synthetic auth data`; `:78` — `does not treat a protected-name prefix as the protected directory`. Note the policy covers `.env` and `.env.*` but **not** `.envrc` or `.environment` (`foundation.md` §7) |
