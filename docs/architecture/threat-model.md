...
| Spec 1 authority is admitted only over the exact V2 handoff, never a partial or repaired one | `admitV2Handoff` (`apps/cli/src/bootstrap/report.ts:1445-1527`) pins the current manifest to exactly one `finalized` envelope through the existing `exactV2Handoff` (`apps/cli/src/bootstrap/report.ts:624-649`). It refuses managed drift with exit 3 (`apps/cli/src/bootstrap/report.ts:178-185`), including a missing ledger or rollback root, because drift runs first. With exit 6 it refuses a finding on the nonce, allocator, active-release or trust record, a missing global lock, nonce/allocator disagreement, a present activation record, a non-empty rollback or recovery-executor reservation, and a non-empty ledger or rollback root. A V1 manifest gets the exit-4 refusal above. The zero-byte rollback and executor reservations fresh `init` creates are admitted as absent records | `apps/cli/src/bootstrap/report.test.ts` — `admits exactly the complete handoff and refuses a non-terminal envelope and each missing member`, one refusal per member with its exit code between two successful admissions, and `refuses a manifest the shipped V1 init produced` |
