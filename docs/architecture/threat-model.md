...
| A V1 manifest is refused, never migrated, once the packaged bootstrap capability exists (Spec 2's D18 amendment) | `init` publishes reason `manifest_v1_not_migratable`, exit 4, recovery `developer-os uninstall, then developer-os init`, through `failureFrom`, before the bootstrap evidence inventory or any plan (`apps/cli/src/commands/init.ts:822-825`); the refusal's `name` is spelled so that `failureFrom` derives exactly that reason on any path (`apps/cli/src/bootstrap/report.ts:184-194`). Without the capability — production until roadmap Phase 4b — the V1 `init` path is unchanged | `apps/cli/src/commands/init.test.ts` — `refuses a shipped V1 installation once the packaged capability is available, and keeps the V1 path without it`, whose V1 home comes from the shipped V1 `init` and whose tree digest is compared across a dry and a real run; `apps/cli/src/bootstrap/report.test.ts` — `refuses a manifest the shipped V1 init produced`, which publishes the admission's rejection through `failureFrom` |
| No command but `init` runs over an interrupted or malformed bootstrap, and retained evidence after a V2 handoff blocks nothing | dispatch routes every other command first (`apps/cli/src/main.ts:514-525`). With a V2 manifest present, the gate reads only `fresh-v2-init.<id>.plan.json` names and never stats a tombstone, payload or lock name (`apps/cli/src/bootstrap/report.ts:1321-1340`); it refuses only a readable non-terminal journal whose plan published the current manifest. Without one, it returns the verdict of the evidence inspection `init` uses — resume, manual archive (including an unreadable namespace), or admit (`apps/cli/src/bootstrap/report.ts:1353-1374`) — and `init` gives the same archive refusal when that inspection cannot read the namespace (`apps/cli/src/commands/init.ts:832-843`). Accepted limit: after the V2 manifest is published, a death before verification plus a later slot replacement is admitted as a handoff | `apps/cli/src/main.test.ts` — `admits every ordinary command when retained evidence goes missing or is altered after a complete V2 handoff` (missing and symlinked tombstones, a symlink inside a retained directory, a symlinked bootstrap lock, emptied, oversized and replaced slots); `refuses every non-init command with init's own archive guidance when an interrupted envelope's plan or slot stops matching`; `refuses only genuine bootstrap residue in a home where bootstrap never ran, with the guidance init gives`; `refuses every non-init command while a fresh V2 envelope is non-terminal, and none after init completes the handoff`; `tests/security/network.test.ts` — `spawn nothing when every non-init command refuses a non-terminal envelope` |
| Spec 1 authority is admitted only over the exact V2 handoff, never a partial or repaired one | `admitV2Handoff` (`apps/cli/src/bootstrap/report.ts:1420-1498`) pins the current manifest to exactly one `finalized` envelope through the existing `exactV2Handoff` (`apps/cli/src/bootstrap/report.ts:621-646`). It refuses managed drift with exit 3 (`apps/cli/src/bootstrap/report.ts:175-182`). With exit 6 it refuses a finding on the nonce, allocator, active-release or trust record, a missing global lock, nonce/allocator disagreement, a present activation record, a non-empty rollback or recovery-executor reservation, and a missing or non-empty ledger or rollback root. A V1 manifest gets the exit-4 refusal above. The zero-byte rollback and executor reservations fresh `init` creates are admitted as absent records | `apps/cli/src/bootstrap/report.test.ts` — `admits exactly the complete handoff and refuses a non-terminal envelope and each missing member`, one refusal per member with its exit code between two successful admissions, and `refuses a manifest the shipped V1 init produced` |
