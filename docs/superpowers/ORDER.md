...
with the two it still has to close — NEW-15 and NEW-22. **Four belong to somebody else:** **NEW-21** is the founder's and blocks
...
honest cost of closing three rows** — a defect that closes cleanly and leaves nothing is rarer than
...
arrived from the same review and closed the same day; NEW-16 closed and left **three**; NEW-11 closed and left NEW-30 and NEW-31; the review that closed it also observed NEW-29, a timing assertion that can redden an unrelated commit. A row leaves §1
when its fix is **committed**, not when its question is answered, so the two R2 still implements —
NEW-15 and NEW-22 — stay listed until it lands them.
