...
|---|---|---|
| NEW-74 | ingest isolation residual / founder decision | **The vendor read scope is now asymmetric and only the founder can settle it.** Roadmap Phase 1 removed Claude's read grant entirely — it runs with `--tools ""` and receives the vault only as a bounded, screened, redacted index excerpt in the prompt. Codex still receives `-C <contentRoot>` with `-s read-only`, and a read-only sandbox reads broadly; `-C` only sets relative-path resolution. So one vendor reads the vault and the other does not, for the same verb. Decide whether Codex's read scope is removed the same way (the excerpt already carries what the model needs), narrowed, or accepted with the reason recorded. Evidence: `packages/adapter-codex/src/invoke.ts` argv, and `docs/architecture/vendor-invocation.md`. |
| NEW-75 | ingest isolation residual / vendor behaviour | **An isolated Claude run still writes into the user's real home.** Observed 2026-09-05 against 2.1.261 with the shipped argv: despite `--no-session-persistence`, one invocation durably creates `.claude.json`, `.claude/.last-cleanup`, a timestamped `.claude/backups/` snapshot, and per-process `.claude/sessions/` `.json` and `.key` files. Because the adapters hand the child `env: {}`, it has no `HOME` and resolves one through `getpwuid_r`, so in production those files land in the developer's own `~/.claude` rather than anywhere this product controls. `tests/integration/ingest/no-user-hooks.test.ts` pins the observed set against a temporary `HOME`. Decide whether ingest must supply a `HOME` it owns — which would mean admitting one environment variable, reversing decision F2 against this recorded observation rather than speculatively — or whether writing into the user's home is acceptable for this verb. Full detail in `docs/architecture/vendor-invocation.md`. |
