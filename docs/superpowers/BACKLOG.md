...
There are 44 numbered rows. They are not automatically ordered ahead of A11.
...
| NEW-77 | platform-macos / CI environment | **The four `SpawnRenameAtxRunner real kernel boundary` cases fail on a GitHub `macos-15` runner and pass on the development laptop.** Run 34133320221: `MacOsRetainedRenameRefusalError` from `packages/platform-macos/src/retained-rename.ts:392`, which is the `result.exitCode !== 0` arm — so `/usr/bin/osascript` was spawned and exited non-zero. **It is not the executable-trust check**, and that is established rather than assumed: `refuses an untrusted osascript before opening the parent` and `requires the fixed osascript capability on a supported macOS host` both passed on the same runner, so the binary is present, root-owned and not group-writable there. What is unknown is which half of the spawned program failed — the JXA `ObjC.bindFunction` bridge being unavailable in a headless CI session, or `renameatx_np` itself refusing on that filesystem. The product reads only the exit code and discards stderr by design, so a refusal cannot distinguish them. **The first diagnostic pass ruled out every obvious cause on the runner** (2026-09-07, run 34152606793): `/usr/bin/osascript` is `-rwxr-xr-x 1 0 0`, the JXA ObjC bridge loads (`getpid()` answered), `renameatx_np` binds, and the work directory is APFS. So the call itself fails. **The live suspect is the flag word.** `RENAME_FLAGS` is the bare literal `0x34` in `packages/platform-macos/src/retained-rename.ts`, carrying no comment and explained nowhere in this repository, and it decomposes to `RENAME_EXCL` (0x04) | `RENAME_NOFOLLOW_ANY` (0x10) | **0x20, which the macOS SDK headers do not define** — `sys/stdio.h` stops at 0x10. Measured 2026-09-07 on Darwin 25.6.0: 0x34, 0x14 and 0x04 all succeed, so that kernel tolerates the undefined bit. The runner is Darwin 24. A second diagnostic step runs the same three flag words there; if 0x34 fails and 0x14 succeeds, the undefined bit is the bug and the product has been depending on one kernel's tolerance for it. Both diagnostic steps must be deleted when this row closes. Close by either making these cases skip with a recorded reason where the capability is absent — the precedent is `e2e/**` and `test:vendor-ingest` having their own jobs — or by establishing that the runner can support them. Do not close it by weakening the refusal: the refusal is correct, since a `renameatx_np` that did not happen must not be reported as success. |
