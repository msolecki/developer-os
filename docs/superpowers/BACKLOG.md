...
|---|---|---|
| A11 · DOS-P7 | Spec 1a (after the NEW-67 amendment), Spec 2 Tasks 10–11 and the production V2 `init` wiring (D16), with NEW-79, NEW-80 and NEW-81 before the pin is removed | current |
...
| NEW-80 | release layout / Phase 4b (D19) | **Fresh `init` writes release metadata and the rollback root where Spec 2 §3.2 does not put them.** §3.2 fixes hash-derived `state/release-metadata/{delegations,indexes,bundles}/<hash>.json` and a product-home `rollback` root (`docs/superpowers/specs/2026-08-28-developer-os-release-update-design.md:266-288`). The executor writes `releases/metadata/<packaged file name>` (`apps/cli/src/bootstrap/executor.ts:2016`) and `state/rollback` (`apps/cli/src/bootstrap/executor.ts:1136`), and the handoff admission hard-codes `state/rollback` (`apps/cli/src/bootstrap/report.ts:1499`). Decision D19: the code follows §3.2, because packaged names cannot hold the active and rollback identities' metadata side by side and the launcher (Task 10) validates hash-derived paths. Close before Task 10 and before the pin is removed, with the exact-set test over a fresh plan's `createdPaths` and `launchabilityPaths` that NEW-67 restores. Free while no V2 installation exists; an on-disk migration after Phase 4b. |
...
| NEW-82 | Spec 1 handoff admission / plan 1a | **`admitV2Handoff` is a fresh-install snapshot, and Spec 1 is its first caller** (Phase 3 final review M2, M3, M7, m4, m5). One `incompleteHandoff` reason cannot tell a non-terminal envelope from no finalized one, and its catch-all hides programming errors; "manifest without a runtime reservation" exercises the hash pin rather than completeness; a directory at `state/lifecycle-activation.json` passes as an absent record, because `guardedFile` returns null for any non-regular entry (`apps/cli/src/bootstrap/report.ts:1437-1443`); and `listNames` is wired to `readdir` instead of being injectable beside `reader` (`apps/cli/src/bootstrap/context.ts:238`). Close in plan 1a, in the change that composes Spec 1's admission under the NEW-67 amendment. |
| NEW-83 | `init` evidence classification / Spec 2 §6.4 | **Bootstrap evidence classification departs from §6.4's words in three places** (Phase 3 reviews). A live `.lifecycle-bootstrap.lock` is filtered out before classification (`apps/cli/src/bootstrap/report.ts:1150-1151`), while §6.4 counts a live bootstrap lock as residue that rules out `unverified` (`docs/superpowers/specs/2026-08-28-developer-os-release-update-design.md:1753-1758`); it is harmless in the one state it admits, a crash before the first payload. Amend §6.4 to "a lock held by a live process" or bind the held lock by descriptor. A `retaining` envelope whose outcome is `rolled_back` is admitted before handoff, and truncated slots after uninstall route to `init`; decide both against §6.4 and pin them. |
...
| NEW-67 | Spec 1 / amendment before execution | Spec 1 as approved prescribes `unlink`/`rmdir`/plan-last compaction (`docs/superpowers/specs/2026-08-21-developer-os-opt-in-surfaces-design.md:624-641`, `:4164-4167`, `:4207-4216`) that the 2026-08-31 retention correction forbids; its `launchctl` row pins macOS 26.5.2 build 25F84 (`:3496-3522`) while the development machine runs a later build, with no fallback (`:1407`); it re-produces types Spec 2 already shipped; its status path `state/automation-<job>.status.json` (`:152`) differs from the implemented `automation-<job>.json`; its closed reservation set (`:143-155`) omits four Spec 2 state files; its three collision codes (`:109-117`) exist nowhere; and several §7 gates require a physical million-entry or 100,000-blob fixture. Founder decision 2026-09-04: split into 1a (config, coordinator, uninstall) and 1b (git, launchd). Amend before Task 1 of either plan. Roadmap Phase 4. **Added 2026-09-17 by the Phase 3 final review:** apply D18 — delete the V1→V2 migration precondition (`docs/superpowers/plans/2026-08-28-developer-os-opt-in-surfaces.md:15-16`), §2.1's migration gate and mapping, the §7 "V2 migration preserves ownership" gate (`docs/superpowers/specs/2026-08-21-developer-os-opt-in-surfaces-design.md:4241`) and §8.2 step 3, so Spec 1 consumes the fresh V2 handoff only; derive the reservation set from Spec 2 §3.2 as D19 fixes it. Two carries: `admitV2Handoff` requires three empty ledgers and an absent activation record (`docs/architecture/foundation.md:470-477`), and after a real handoff an emptied or replaced retained slot makes it refuse permanently while the ordinary-command gate admits. The D18 revert deleted the fresh-plan reserved-path pin test; restore it from `git show df3e947 -- apps/cli/src/bootstrap/executor.test.ts` as the amendment's first test. **Drafted 2026-09-17** at `docs/superpowers/specs/2026-09-17-spec-1-new-67-amendment.md`, awaiting founder approval. |
