...
There are 32 numbered rows. They are not automatically ordered ahead of A11.
...
| NEW-57 | A11 / Task 6 / crash recovery | **The uncommitted Task 6 implementation regresses crash recovery at four fine-grained death points that pass at `a3ad015`.** Measured 2026-09-04 by running `apps/cli/src/bootstrap/executor.test.ts` on three trees: pristine HEAD passes, the Task 6 working tree fails, and the same failures survive today's corrections — so they belong to the Task 6 WIP from earlier sessions, not to the retained-evidence fix. `during_initial_slot_write` is **closed**: a death during the first journal write leaves slot 0 partial and slot 1 empty, `decodeCanonicalJson` threw out of `inspectPlan`, and once guarded the envelope was classed `unverified`, so a later `init` opened a second envelope and the test saw two durable plans. It now resumes when no slot is authority, which is the `after_plan` state generalised to a partial first write. Still open: `after_global_lock_create`, `before_global_lock_parent_sync` and `after_global_lock_parent_sync` all resume to exit 6 `existing global lock escaped admitted rolled-back evidence`. `exactReusableGlobalLock` at `apps/cli/src/bootstrap/report.ts` admits an existing global lock only from `createdPathEvidence` at the lock's own `createdPaths` ordinal, and at these death points the lock inode exists while its creation evidence was never made durable — so recovery cannot prove the lock is its own and refuses, though the executor is the only writer of that path under the held bootstrap lock. Deciding what admits a plan-owned global lock whose creation evidence did not survive is a Spec 2 §6.1/§6.4 question, not an implementation choice. Reviewer A raised this class on 2026-09-04 and it was wrongly dismissed as contradicted by a prior gate log; the prior log never covered these rows. |
| NEW-56 | A11 / Task 6 / residuals | The three gates that could not fail are closed in the working tree: `vendorProcesses` now records `executable` plus args in the fixture runner before delegating; `releaseRequests` was removed because `PackagedReleaseSourceV1` carries no interceptable method, so add a release-fetch seam with the update surface in Spec 2 Tasks 8+ where a fetch exists to observe; `status.test.ts` builds its fixture with `bootstrapAvailable: true` so the inertness counter is live rather than pinned to 0; and a new `inventoryDigest` carries a SHA-256 per regular file, so the seven byte-inertness claims in `report.test.ts`, `doctor.test.ts`, `init.test.ts` and `uninstall.test.ts` would now fail against an in-place rewrite. Still open: reads no longer enforce the retention cap — `init` still refuses before allocating, at both preflight and under the lock — but no test pins the read path, because genuine over-cap residue needs 257 envelopes, 1,000,000 entries or 12 GiB and no fixture builds one. No test creates two admitted active plans, so the `active.length > 1` term of `blocksNewIntent` is unexercised. `apps/cli/src/bootstrap/report.ts` re-implements Core's retention table rather than sharing a derivation: its participant filter uses `terminal.nextFoundationParticipant > 0` where Core uses a per-participant ordinal, and its compensation exclusion list omits the bootstrap lock, plan and journal-slot paths Core excludes — both unreachable while the executor builds exactly one forward participant, and both the class of defect that produced NEW-55. `preserved` carries every recursive descendant of every directory tombstone, printing Brain note filenames on uninstall and costing one `canonicalize` per retained entry. |
| NEW-55 | A11 / Task 6 / implemented, uncommitted | **Closed by founder decision on 2026-09-04 and implemented; not yet committed.** A successful `developer-os init` was retiring 14 of its own 105 manifest artifacts — `config.toml`, `schemas/ingest.stage.schema.json`, and twelve files inside the user's Brain — by renaming them to tombstones, then exiting 0 with a manifest claiming all 105 installed. Cause: `foundationAuthorities` named `mutation.targetPath` as a `foundation_bootstrap` row for a forward participant, mirrored in `apps/cli/src/bootstrap/report.ts`, and retention runs after `verify()`. Spec 2 §6.4 line 1601 forbids it. The founder settled the replacement: a finalized forward participant emits no mutation content row. Implemented as an `installedTarget` flag that stays in the authority list for bookkeeping — so `foundationByPayload` still suppresses the `payload` row it owns — and is filtered where rows are emitted, in `authorities()`, the one producer both `deriveBootstrapRetentionLocations` and `deriveBootstrapRetentionTableUncached` read. Rollback is byte-identical: the flag is false unless the outcome is finalized. Verified: reproduction now reports 0 absent artifacts and 0 Brain tombstones against 14 and 12 before; `init.test.ts` "leaves every artifact its manifest names present after a V2 bootstrap init" was proved red first and passes; Core 300, platform-macos 63, e2e plus security 13, CLI 285, `init.test.ts` 38 all pass; fresh review returned READY. It stays uncommitted only because `npm run check` is red on NEW-57. Patch: `task6-session4.patch` with `report.session4.ts` at `~/.claude/projects/-Users-msolecki-www-developer-os/task6-patches/`. |
