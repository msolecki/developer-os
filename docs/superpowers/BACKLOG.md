...
| NEW-55 | A11 / Task 6 / blocked by NEW-53 | Replacement Task 6 is parked. Verified working: the bootstrap lock blocks on shape rather than bare existence, a published plan whose two journal slots are still empty is resumable, and `FreshV2InitPlanV1.admittedPreexistingPaths` persists the preflight set so a process resuming after a crash no longer fails with `bootstrap preflight authority is absent`. Two items remain: a fourth `init` reports `managed artifacts differ from their recorded state`, and the test runs two full inits at roughly 220s each against its own 300000ms budget, so it cannot pass until NEW-53 closes. Work is preserved with a restore guide at `~/.claude/projects/-Users-msolecki-www-developer-os/task6-patches/`. |
| NEW-53 | A11 / performance / blocks Task 6, CI and push | `developer-os init` is still far too slow: it must fit two inits inside a 300000ms test budget and the whole suite inside `check.yml`'s 30 minutes. Encoder allocations per init are down from 366,721,268 to 96,995,192 across b146f7e and ae12887 by ordering keys once, memoizing the plan hash and the retention table, and removing a double encode per evidence row. Remaining attribution, measured by counting `encodeCanonicalJson` calls during one init: 968,031 calls in total, dominated by `verifyAuthority` under `deriveBootstrapRetentionTable`, then `canonicalNoLf` under `canonicalHash`, then `sameValue` which compares by canonical encoding. The table is derived 1,475 times per init and the identity cache only partially hits because evidence projections are rebuilt; making those projections stable, or deriving the table once per execution, is the next lever. |
