...
|---|---|---|
| A11 · DOS-P7 | ingest isolation, bootstrap performance and the first push, manifest/new-init handoff, Spec 1 implementation, remaining Spec 2 implementation | current |
...
There are 45 numbered rows. They are not automatically ordered ahead of A11.

| ID | Owner / blocker | Work required to close |
|---|---|---|
| NEW-58 | ingest isolation / security / startable, before any other A11 work | `invokeClaude` (`packages/adapter-claude/src/invoke.ts:105-115`) passes `--allowedTools`, which grants tools rather than restricting them, and passes none of `--tools`, `--restricted`, `--permission-mode`, `--strict-mcp-config` or `--setting-sources`, so an ingest run executes with the user's own permission settings, hooks and MCP servers loaded; the read-only intent at `apps/cli/src/commands/ingest.ts:228` and `apps/cli/src/commands/ingest.ts:768-784` is not enforced. Founder decision 2026-09-04: ingest runs with no tools; the prompt carries the capture plus a bounded index excerpt. Verified against the installed binaries 2026-09-04 (`docs/superpowers/plans/2026-09-04-developer-os-ingest-isolation.md`, Founder decisions table): `--tools ""` exists and disables all tools; `--strict-mcp-config` exists and, with no `--mcp-config`, loads zero MCP servers, confirming the stale-comment claim that Claude has no schema flag was already wrong — `--json-schema` exists in 2.1.260. `--permission-mode` exists with six values but the help gives no per-value description, so "the most restrictive permission mode" is **not determinable from the help**; `--setting-sources` exists but whether `""` means "load none" is **not stated anywhere**. Open and unresolved: `--max-turns` does not appear anywhere in `claude --help` for 2.1.260, yet `packages/adapter-claude/src/invoke.ts:105-115` passes it on every invocation — either the flag is undocumented, or **every real Claude ingest run today fails on an unknown flag**. This may be a live defect, not a preference; the plan's Task 1 Step 2 settles which case it is before Task 2 begins. On Codex: `invokeCodex` (`packages/adapter-codex/src/invoke.ts:265-276`) omits `--ephemeral`, `--ignore-user-config` and `--ignore-rules`, so a run persists a thread in the user's history and loads the user's config, rules and MCP servers; verified 2026-09-04 that all three exist **only on `codex exec`**, not top-level, and `--ignore-rules` covers execpolicy `.rules` only, not hooks — codex 0.151.0 has **no** per-invocation flag disabling MCP servers, only `--ignore-user-config`, which stops `config.toml` loading entirely. The claim that `finalAgentMessage` (`packages/adapter-codex/src/invoke.ts:178-200`) has a "deterministic replacement" in this version's `turn.completed` `last_agent_message` is **not established**: `codex app-server generate-json-schema` shows that protocol's `TurnCompletedNotification` carries no such field — an agent reply there is a `ThreadItem` `{type: "agentMessage", text}` — but app-server is JSON-RPC, a different interface from `codex exec --json`'s JSONL stream, so this is suggestive, not conclusive. No fixture comes from 0.151.0 (with NEW-45/NEW-47). Plan written 2026-09-04, not yet founder-approved: `plans/2026-09-04-developer-os-ingest-isolation.md`. Roadmap Phase 1. |
| NEW-72 | ingest isolation / roadmap correction | Roadmap Phase 1's "process environment allowlist" bullet (`docs/superpowers/plans/2026-09-04-developer-os-completion-roadmap.md:48`) is a **widening**, not a hardening. Both adapters pass `env: {}` today, so the vendor child gets a completely empty environment; `tests/security/network.test.ts:72` pins that, and its docblock says the empty environment "is stricter than spec §2.7 asks and this constant is where a vendor CLI that genuinely needs `HOME` would be admitted, deliberately, in one place a reviewer can see." The roadmap also names proxy variables; `tests/security/network.test.ts:228` (`"does not pass a proxy the parent process was given"`) exists to prove a parent's proxy does not reach the child, so admitting one would contradict a standing security test. Close by admitting a variable only against a recorded observation of the vendor failing without it (`plans/2026-09-04-developer-os-ingest-isolation.md` Task 6), or by recording the decision to keep the empty environment. Roadmap Phase 1. |
...
- [ ] Roadmap Phase 1 plan `plans/2026-09-04-developer-os-ingest-isolation.md` is written, not yet
  founder-approved: isolates the ingest invocation on both vendors (NEW-58) and resolves
  nested-session attribution (NEW-44). Three founder decisions (F1, F2, F3) block Task 2.
