...
|---|---|---|
| A11 · DOS-P7 | Spec 1a (plan 1a against Spec 1 as amended 2026-09-17), Spec 2 Tasks 10–11 and the production V2 `init` wiring (D16), with NEW-79, NEW-80 and NEW-81 before the pin is removed | current |
...
| NEW-80 | release layout / Phase 4b (D19) | **Fresh `init` writes release metadata and the rollback root where Spec 2 §3.2 does not put them.** §3.2 fixes hash-derived `state/release-metadata/{delegations,indexes,bundles}/<hash>.json` and a product-home `rollback` root (`docs/superpowers/specs/2026-08-28-developer-os-release-update-design.md:268-290`). The executor writes `releases/metadata/<packaged file name>` (`apps/cli/src/bootstrap/executor.ts:2016`) and `state/rollback` (`apps/cli/src/bootstrap/executor.ts:1136`), and the handoff admission hard-codes `state/rollback` (`apps/cli/src/bootstrap/report.ts:1499`). Decision D19: the code follows §3.2, because packaged names cannot hold the active and rollback identities' metadata side by side and the launcher (Task 10) validates hash-derived paths. Close before Task 10 and before the pin is removed, with the exact-set test over a fresh plan's `createdPaths` and `launchabilityPaths` that plan 1a restores. Free while no V2 installation exists; an on-disk migration after Phase 4b. |
