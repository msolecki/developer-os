...
There are 41 numbered rows. They are not automatically ordered ahead of A11.
...
| NEW-52 | test gate / CI budget — closed 2026-09-08 | Closed with its own description corrected. It said `check.yml` runs "four parallel jobs with 20/180/40/40 minute bounds"; there are **five** since `74a5b02` added `vendor-ingest`, and the bounds are now 20/330/150/40/15. Both long bounds were resized twice on 2026-09-07 from a **measured** runner ratio rather than a guess: identical retained-evidence cases take 112-120 s locally and 237,571 ms / 208,545 ms on `macos-15`, so ~1.9-2x. `suite` needed ~88 min against a 40 then a 75 minute bound and was killed twice; `bootstrap-executor` measured **248 min** in run 34133320221 against the old 180 and was raised to 330 pre-emptively from that same ratio, which is the only reason it ever completed. The residual is NEW-53's: these bounds are indefensible as a steady state and only shrink when an `init` stops costing ~101 s. |
